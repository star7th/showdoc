<template>
  <div class="filehub-upload-modal">
    <CommonModal
      :class="{ show }"
            ...
            >
              ...
              <div class="status-info">
                ...
                >
                  ...
                  <span>{{ $t('page.file_upload_success') }}</span>
                </span>
...
const uploadSingleFile = async (file: File) => {
  ...
  try {
    const data = await request('/api/page/upload', formData, 'post', true)
    ...
    if (currentItem) {
      if (data.success === 1) {
        ...
        currentItem.error = data.error_message || t('page.upload_failed')
      }
      uploadedCount.value++
    }
...
.filehub-upload-modal :deep(.modal-content) {
  padding: 20px;
}
