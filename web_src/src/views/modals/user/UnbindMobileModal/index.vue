<template>
  <div class="unbind-mobile-modal">
    <CommonModal
      :class="{ show }"
      :title="$t('user.unbind_mobile')"
      @close="handleClose"
    >
      <div class="modal-content">
        <div class="info-text">
          {{ $t('user.unbind_mobile_tips') }}
        </div>
        <div class="form-item">
...
const handleSubmit = async () => {
  ...
  try {
    await request('/api/user/unbindMobile', {
      password: formData.value.password
    })
    // 使用弹窗提醒用户
    AlertModal(t('user.unbind_mobile_success'), {
      callback: () => {
        props.onClose()
      }
    })
