...
export function toggleNthTaskCheckbox(markdown: string, index: number, checked: boolean): string {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (seen + partCount >= targetGlobalIdx) {
      ...
      let count = 0
      part.text = part.text.replace(replaceRegex, (all: string, p1: string, p2: string) => {
        count += 1
        if (count === nthInPart) {
