...
class PageController extends BaseController {
    ...
    public function ajaxHttpApi(){
        ...
        if($method=='get'){
            $url=$url."?".$params;
            $return=$this->http_get($url, $cookies);
        }else{
            $return=$this->http_post($url, $params, $cookies);
        }
        echo $return;
    }
    /**
     ...
     */
    private function http_get($url, $cookies="") {
        $oCurl = curl_init ();
        ...
        if(!empty($cookies)){
            ...
            curl_setopt ( $oCurl, CURLOPT_COOKIE, $cookies);
        }
     ...
     * @return string content
     */
    private function http_post($url, $param, $cookies="") {
        $oCurl = curl_init ();
        if (stripos ( $url, "https://" ) !== FALSE) {
            curl_setopt ( $oCurl, CURLOPT_SSL_VERIFYPEER, FALSE );
            curl_setopt ( $oCurl, CURLOPT_SSL_VERIFYHOST, false );
